...
export class ObjectStorageService {
  getPublicObjectSearchPaths(): string[] {
    ...
    if (paths.length === 0) {
      throw new Error(
        "PUBLIC_OBJECT_SEARCH_PATHS is not configured. Set up App Storage first.",
      );
    }
    return paths;
  }
  ...
  getPrivateObjectDir(): string {
    ...
    if (!dir) {
      throw new Error(
        "PRIVATE_OBJECT_DIR is not configured. Set up App Storage first.",
      );
    }
    return dir;
  }
...
async function signObjectURL({
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to sign object URL (${response.status}); the API must run on Replit.`,
    );
  }
